...
@section('header', 'Organization Users')

@section('content')
    <div class="bg-white rounded-xl shadow-sm border border-[#e3e3e0] p-6 md:p-8 space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold text-[#1b1b18]">Organizational Members</h2>
                <p class="text-sm text-[#706f6c] mt-1">
                    Manage organization member accounts. You can add, edit, and archive members.
                </p>
            </div>

            <a href="{{ route('admin.organization-users.create') }}"
        ...
        <form method="GET" class="grid grid-cols-1 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-4 items-end">
            ...
            <div class="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_auto] gap-3">
                <div>
                    <label for="committee_id" class="block text-xs font-medium text-gray-600 mb-1">Committee</label>
                    <select id="committee_id" name="committee_id"
                        ...
                        @foreach ($committees as $committee)
                            <option value="{{ $committee->id }}" @selected(($filters['committee_id'] ?? null) == $committee->id)>
                                {{ $committee->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <table class="min-w-full divide-y divide-[#ecebe7]">
                <thead class="bg-[#faf9f5]">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3.5 text-left text-xs font-semibold text-gray-600">Email</th>
                        <th scope="col" class="px-4 py-3.5 text-left text-xs font-semibold text-gray-600">Position</th>
                        <th scope="col" class="px-4 py-3.5 text-left text-xs font-semibold text-gray-600">Permission</th>
                        <th scope="col" class="px-4 py-3.5 text-left text-xs font-semibold text-gray-600">Committee</th>
                ...
                <tbody class="divide-y divide-[#f2f1ec] bg-white">
                    @forelse ($users as $user)
                        <tr class="hover:bg-[#faf9f5]">
                            <td class="px-4 py-3 text-sm text-[#1b1b18] font-medium">{{ $user->name }}</td>
                            <td class="px-4 py-3 text-sm text-[#4b4a47]">{{ $user->email }}</td>
                            <td class="px-4 py-3 text-sm text-[#4b4a47]">{{ $user->position }}</td>
                            ...
                            <td class="px-4 py-3 text-sm text-right">
                                <div class="inline-flex items-center gap-2">
                                    <a href="{{ route('admin.organization-users.edit', $user) }}"
                                    ...
                                    <button type="button" @click="$dispatch('open-confirmation-modal', {
                                                                    ...
                                                                    confirmText: 'Archive',
                                                                    actionUrl: '{{ route('admin.organization-users.destroy', $user) }}',
                                                                    actionMethod: 'DELETE'
                                                                })"
